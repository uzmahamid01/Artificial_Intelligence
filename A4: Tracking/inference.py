...
def constructBayesNet(gameState: hunters.GameState):
    ...
    #populate `edges` with every edge in the Bayes Net. we will represent each edge as a tuple `(from, to)`.
    edges = [(PAC, OBS0),(PAC, OBS1), (GHOST0, OBS0), (GHOST1, OBS1)]  
    
    #set each `variableDomainsDict[var] = values`, where `values` is a list of the possible assignments to `var. each agent position is a tuple (x, y) where x and y are 0-indexed each observed distance is a noisy Manhattan distance:. it's non-negative and |obs - true| <= MAX_NOISE
    ...
    max_distance = gameState.getWalls().width - 1 + gameState.getWalls().height - 1
    # Define correct domain for observations (0 to max_distance + MAX_NOISE, inclusive)
    variableDomainsDict[OBS0] = list(range(0, max_distance + MAX_NOISE + 1))
    variableDomainsDict[OBS1] = list(range(0, max_distance + MAX_NOISE + 1)) 
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        def joinAndEliminate(factors, variable):
            factors, joinedFactor = joinFactorsByVariable(factors, variable)
            if len(joinedFactor.unconditionedVariables()) > 1:
                return factors + [eliminate(joinedFactor, variable)]
            return factors

        factors = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        # Interleave joining and eliminating
        for variable in eliminationOrder:
            factors = joinAndEliminate(factors, variable)

        #join 
        joinedFactor = joinFactors(factors)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        prob = random.uniform(0, self.total())
        cumulative_sum = 0.0
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition and noisyDistance is None:
            return 1
        if noisyDistance is not None and ghostPosition != jailPosition:
            # Calculate distance to pacman through walls
            man_dist = manhattanDistance(pacmanPosition, ghostPosition)
            return busters.getObservationProbability(noisyDistance, man_dist)
        return 0
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        observation_probs = [self.getObservationProb(observation, pacman_position, ghost_position, jail_position)
                            for ghost_position in self.allPositions]
        self.beliefs = DiscreteDistribution({pos: prob * self.beliefs[pos] for pos, prob in zip(self.allPositions, observation_probs)})
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        legalPositions = self.allPositions
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        remainder_particles = self.numParticles % num_positions

        for position in self.legalPositions:
            self.particles.extend([position] * particles_per_position)

        # Distribute remainder particles evenly across positions
        for i in range(remainder_particles):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            prob_obs_given_particle = self.getObservationProb(observation, pacmanPosition, particle, jailPosition)
            weight_distribution[particle] += prob_obs_given_particle

        #see if all particles have zero weight
